    ...
    try {
        for await (const raw of parser) {
            ...
            try {
                ...
                log(`  ✖  Failed to import production row (ID: ${row.ID ?? "unknown"}): ${err}`);
                failed++;
            }
        }
    } catch (err) {
        log(`  ✖  CSV parse error, import may be incomplete: ${err}`);
    }

    log(
        `Productions done — created: ${created}, updated: ${updated}, skipped: ${skipped}, failed: ${failed}`
    );
}
...
async function importEvents(filePath: string) {
    ...
    try {
        for await (const raw of parser) {
            ...
            try {
                ...
                log(`  ✖  Failed to import event row (Production: ${row.Production ?? "unknown"}, Start: ${row.Starttime ?? "unknown"}): ${err}`);
                failed++;
            }
        }
    } catch (err) {
    ...
    log(
        `Events done — created: ${created}, updated: ${updated}, skipped: ${skipped}, failed: ${failed}`
    );
}
